...
и состоит из независимых packages, которыми управляет новый `cozystack-operator`.
...
Перед обновлением просмотрите этот список, чтобы понять влияние на ваши workload.
...
Приложение `mysql` переименовано в `mariadb`, чтобы точнее отражать используемый database engine.
...
например `mysql-mydb-primary.<namespace>.svc`, после миграции нужно обновить connection
strings и использовать новый префикс `mariadb-`.
{{% /alert %}}
...
- **vm-disk** управляет disk images виртуальных машин.
...
Стек monitoring был реструктурирован. HelmRelease с именем `monitoring` в каждом
tenant namespace мигрируется в новый release с именем `monitoring-system`.

Миграция выполняется автоматически: все monitoring resources (VictoriaMetrics, Grafana, Alerta,
...
Следующие внутренние изменения не влияют напрямую на application workload, но важны
для automation scripts или custom tooling, которые взаимодействуют с внутренними компонентами Cozystack:
